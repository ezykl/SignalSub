  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      <View className="px-4 pt-3 pb-2 flex-row items-center justify-between">
        <Text className="text-2xl font-bold font-heading text-white">Profile & Analytics</Text>
        <TouchableOpacity
      ...
      >
        ...
        <View className="mx-4 mt-2 mb-4 p-4 rounded-2xl bg-card border border-surface flex-row items-center justify-between">
          <View className="flex-row items-center gap-3">
            ...
            <View>
              <Text className="text-base font-bold font-heading text-white">{userName}</Text>
              <Text className="text-xs text-muted font-body mt-0.5">
                {activeSubs.length} active {activeSubs.length === 1 ? 'subscription' : 'subscriptions'}
              </Text>
            </View>
          </View>
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text className="text-xs font-semibold text-[#A78BFA] font-heading">Settings</Text>
            <MaterialIcons name="chevron-right" size={16} color="#A78BFA" />
          </TouchableOpacity>
        </View>

        {/* Analytics Section Header */}
        <View className="px-4 mt-2 mb-2">
          <Text className="text-lg font-bold font-heading text-white">Spending Analytics</Text>
        </View>

        {/* Period Selector */}
        <View className="flex-row bg-card rounded-xl mx-4 mb-4 p-1 border border-surface" testID="period-selector">
        ...
        {activeSubs.length === 0 ? (
          <View className="bg-card rounded-2xl p-6 mx-4 mt-2 items-center justify-center border border-surface" testID="analytics-empty-state">
            ...
            <Text className="text-[13px] text-muted text-center leading-[18px] max-w-[260px] mb-5 font-body">
              Add active subscriptions to view your spending intelligence, category breakdown, and monthly trends.
            </Text>
            <TouchableOpacity
          ...
          <View className="px-4" testID="analytics-content">
            ...
            {stats.mostExpensive && (
              <View className="mb-6">
                <Text className="text-base font-bold font-heading text-white mb-3">Subscription Insights</Text>
                <View className="flex-row gap-3">
                  <View className="flex-1 bg-card rounded-2xl p-4 border border-surface">
                    <Text className="text-xs text-muted font-body mb-1">Highest Cost</Text>
                    <Text className="text-sm font-bold text-white font-heading" numberOfLines={1}>
                      {stats.mostExpensive.name}
                    </Text>
                    <Text className="text-sm font-semibold text-accentPurpleLight font-heading mt-1">
                      {currency} {stats.mostExpensive.amount.toFixed(2)} / {stats.mostExpensive.billingCycle}
                    </Text>
                  </View>
                  {stats.cheapest && (
                    <View className="flex-1 bg-card rounded-2xl p-4 border border-surface">
                      <Text className="text-xs text-muted font-body mb-1">Lowest Cost</Text>
                      <Text className="text-sm font-bold text-white font-heading" numberOfLines={1}>
                        {stats.cheapest.name}
                      </Text>
                      <Text className="text-sm font-semibold text-accentPurpleLight font-heading mt-1">
                        {currency} {stats.cheapest.amount.toFixed(2)} / {stats.cheapest.billingCycle}
                      </Text>
                    </View>
                  )}
                </View>
        ...
        <View className="px-4 mt-2">
          <Text className="text-base font-bold font-heading text-white mb-3">Preferences & Controls</Text>
          <View className="bg-card rounded-2xl border border-surface overflow-hidden">
            ...
            >
              <View className="flex-row items-center gap-3">
                <View className="w-8 h-8 rounded-full bg-primary/20 items-center justify-center">
                  <MaterialIcons name="attach-money" size={18} color={COLORS.accentPurpleLight} />
                </View>
                <View>
                  <Text className="text-sm font-semibold text-white font-heading">Default Currency</Text>
                  <Text className="text-xs text-muted font-body">{currency}</Text>
                </View>
              </View>
            ...
            >
              <View className="flex-row items-center gap-3">
                <View className="w-8 h-8 rounded-full bg-primary/20 items-center justify-center">
                  <MaterialIcons name="notifications-none" size={18} color={COLORS.accentPurpleLight} />
                </View>
                <View>
                  <Text className="text-sm font-semibold text-white font-heading">Notifications & Reminders</Text>
                  <Text className="text-xs text-muted font-body">Manage renewal alerts</Text>
                </View>
              </View>
              <MaterialIcons name="chevron-right" size={20} color="#94A3B8" />
            </TouchableOpacity>
            ...
            >
              <View className="flex-row items-center gap-3">
                <View className="w-8 h-8 rounded-full bg-primary/20 items-center justify-center">
                  <MaterialIcons name="tune" size={18} color={COLORS.accentPurpleLight} />
                </View>
                <View>
                  <Text className="text-sm font-semibold text-white font-heading">Theme & Personalization</Text>
                  <Text className="text-xs text-muted font-body">Avatar, dark theme, OLED</Text>
                </View>
              </View>
              <MaterialIcons name="chevron-right" size={20} color="#94A3B8" />
            </TouchableOpacity>
